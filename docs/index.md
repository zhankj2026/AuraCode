...
| 维度 | 说明 |
|------|------|
| **56 个内置工具** | 文件读写、代码搜索、LSP 智能、终端执行、Web 搜索、子代理协同、定时任务等 |
| **54 条交互命令** | 涵盖 Git 工作流、会话管理、调试诊断、安全审查、性能基准、插件管理等 |
| **9 项领域技能** | 按需激活的领域知识包，渐进式披露，节省 Token |
| **MCP 协议集成** | 完整客户端 + 服务器模式 — 自动发现、动态注册、企业安全策略 |
| **Bridge 远程控制** | REST + WebSocket — 多会话管理、实时事件推送（事件级 + Token 级）、远程审批 |
| **记忆系统** | LLM 驱动召回 — 新鲜度衰减、语义搜索、跨会话持久化、自动提取 |
| **6 层错误恢复** | 纵深防御 — Fallback 模型 → Prompt 压缩 → 截断恢复 → Hook 兜底 → 预算控制 → 图片剥离 |
...
- [命令参考](user-guide/commands.md) — 54 条命令完整说明
- [工具系统](user-guide/tools.md) — 56 个内置工具分类介绍
