...
### 1.1 背景
opencode 当前已实现类 Claude Code 的 AI 编程助手核心能力（完成度约 99.5%），具备强大的会话管理、工具调用、代码理解、子代理协同能力。但其交互方式以 CLI 为主，缺乏面向软件工程全生命周期的系统性管理界面、流程编排及外部生态集成能力。
...
|------|------|------|------|------|------|------|----------|
| 产品经理 (PM) | ✅ 读写 | 👁️ 只读 | 👁️ 只读 | 👁️ 只读 | 👁️ 只读 | ✅ 审批 | ❌ |
| 架构师 | 👁️ 只读 | ✅ 读写 | 👁️ 只读 | 👁️ 只读 | 👁️ 只读 | 👁️ 只读 | ❌ |
| 开发工程师 | 👁️ 只读 | 👁️ 只读 | ✅ 读写 | ✅ 提交缺陷 | ✅ 预发布 | ❌ | ❌ |
...
|----|--------|----------|---------|------------------|
| REQ-01 | 需求录入与解析 | 支持 Markdown 编辑、PRD 文档上传（PDF/Word），AI 自动抽取结构化字段（Epic/Feature/User Story） | ✅ NLP 结构化 | 从 Jira 导入需求 |
...
| REQ-04 | 冲突与遗漏检测 | 分析需求条目间的语义矛盾及边缘场景覆盖度 | ✅ 规则引擎+LLM | - |
...
| DES-03 | 索引智能推荐 | 分析代码中的 ORM 查询模式（如 Django/Spring JPA），推荐最优索引组合 | ✅ 代码扫描+索引顾问 | - |
...
| DES-07 | 设计评审 | 自动检查设计是否遵循规范（命名、安全、高可用），检测架构反模式 | ✅ 规则引擎+LLM | 发送评审通知至钉钉/飞书 |
...
| DEV-03 | 代码审查 | 对变更集自动进行安全漏洞（16类）、代码异味、逻辑错误扫描 | ✅ SecurityScanner+CodeAnalyzer | 关联 GitLab MR/PR 评论 |
...
| TST-02 | 测试执行与监控 | 触发单元/集成/E2E 测试，实时流式输出执行日志 | ✅ TestRunner | 对接 Jenkins 测试插件 |
| TST-03 | 失败智能分析 | 分析失败堆栈，分类（断言/超时/环境/代码缺陷）并推荐修复方案 | ✅ 日志分析+LLM | 自动创建 Jira 缺陷单 |
| TST-04 | 覆盖率趋势 | 收集行/分支/函数覆盖率，生成版本间趋势图 | ✅ CoverageTracker | - |
| TST-05 | 缺陷管理与追踪 | AI 辅助填写缺陷标题、严重级别、复现步骤，关联需求与代码变更 | ✅ LLM | 双向同步 Jira/TAPD |
| TST-06 | 性能瓶颈预测 | 基于代码圈复杂度与调用链，预测潜在性能热点 | ✅ CodeAnalyzer | - |
...
| DEP-03 | 流水线编排 | 可视化 DAG 编排部署步骤（构建→迁移→发布→探测） | - | 对接 Jenkins/Tekton |
| DEP-04 | 一键部署与回滚 | 选择环境/版本执行部署，支持应用+数据库双回滚 | - | 调用 K8s API / Ansible |
| DEP-05 | 健康检查与金丝雀 | 部署后自动执行健康探测，支持灰度发布智能放量（基于监控指标） | ✅ 异常检测 | 对接 Prometheus/Grafana |
...
| ACC-03 | 追溯矩阵补全 | 基于 Git 提交记录、测试用例执行历史，自动补全缺失的需求关联 | ✅ 代码分析 | - |
...
| **质量检测** | 需求冲突、设计反模式、安全漏洞、代码异味、配置错误 | 需求、设计、开发、部署 |
| **自动修复** | 简单测试失败自动修复、配置参数自动补全 | 测试、部署 |
| **智能决策** | 灰度放量、回滚策略、测试优先级排序 | 测试、部署 |

---

## 5. 集成网关（Integration Gateway）架构需求

为实现“生态互联”，平台须引入轻量级集成网关，参考 Hermes Gateway 设计模式。
...
- **RBAC**：至少 7 个角色（PM/Arch/Dev/Tester/Ops/PMgr/Admin）。
...
- [x] 项目管理与六阶段状态机
- [x] 需求 CRUD + 基础拆解
- [x] 代码开发核心（WebSocket 会话 + 文件读写 + 搜索）
- [x] 数据库模型管理（ER 图 + DDL 生成）
- [x] 一键部署 + 日志查看（单环境）
- [x] 模型配置与基础权限
- [x] **出站通知适配器（钉钉/飞书推送）**

### P1 智能增强与协同（第 3-4 月）
- [ ] 全阶段 AI 生成能力（设计文档/测试用例/验收报告）
- [ ] 影响分析（需求→代码，DDL→应用）
- [ ] 代码审查 + 依赖图可视化
- [ ] 覆盖率趋势与缺陷管理
- [ ] 部署回滚 + 审批流
- [ ] Git Worktree + Cron 定时任务
- [ ] **入站适配器（Jira Webhook → 触发影响分析）**
- [ ] 需求追溯矩阵

### P2 生态互联与高级特性（第 5-6 月）
- [ ] 完整网关事件路由表（自定义条件触发）
- [ ] 双向同步（Jira/飞书命令→平台操作）
- [ ] 灰度发布智能放量（对接 Prometheus）
...
| 维度 | 指标 | 目标值 |
|------|------|--------|
| **效率** | 需求→部署全流程耗时 | 较传统模式缩短 **40%** |
| **质量** | AI 生成代码审查通过率 | ≥ 75% |
| **覆盖** | 自动生成测试覆盖率 | ≥ 80%（新项目） |
| **采用** | 团队成员日活 | ≥ 80% 研发人员 |
| **集成** | 外部系统对接数量 | ≥ 5 个（Jira/钉钉/飞书/Jenkins/GitLab） |
| **AI** | LLM Token 成本/需求 | 控制在 $0.5 以内 |
...
| **API 设计全面** | 91 个端点覆盖所有业务域，RESTful 设计规范 |
...
| 缺失项 | 重要度 | 说明 | 建议补充 |
|--------|--------|------|----------|
| **通知中心** | 高 | 91 个端点中无"通知"端点，用户无法知道任务更新 | 增加 `/notifications` 端点（列表/已读/WebSocket 推送） |
