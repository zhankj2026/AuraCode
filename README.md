...
| **定时任务** | Cron 调度 — 自然语言解析、周期执行、立即执行、30 天自动过期 |
| **动态工作流** | 批量编排 — 任务分解、并行执行、进度追踪、错误恢复 |
...
基于 Cron 调度的周期性任务编排系统，支持自然语言间隔解析和立即执行。

### 核心能力

- ✅ **自然语言解析** — `5m`, `30m`, `2h`, `1d` 等灵活格式
- ✅ **智能间隔识别** — 前导 token、尾部 `every` 子句、默认值三级解析
- ✅ **Cron 表达式转换** — 自动转换为标准 5 字段 Cron 表达式
- ✅ **立即执行** — 调度后立即执行第一次，不等 Cron 触发
- ✅ **30 天自动过期** — 周期性任务 30 天后自动停止
- ✅ **斜杠命令支持** — `/loop 5m /verify` 直接调度斜杠命令
...
| 优先级 | 规则 | 示例 |
|--------|------|------|
| 1 | 前导 token | `5m /verify` → 间隔 `5m`，prompt `/verify` |
| 2 | 尾部 `every` 子句 | `check deploy every 20m` → 间隔 `20m` |
| 3 | 默认间隔 | `check deploy` → 间隔 `10m` |
...
- `/cron` — 查看所有定时任务
- `/cron delete <job_id>` — 取消特定任务

---

## 🔄 动态工作流（Batch 特性）

大规模并行变更编排系统，支持任务分解、并行执行和进度追踪。

### 核心能力

- ✅ **任务分解** — 将大规模变更拆分为可并行执行的子任务
- ✅ **并行执行** — 多代理协同工作，同时处理多个文件
- ✅ **进度追踪** — 实时显示任务完成状态
- ✅ **错误恢复** — 部分失败时自动重试或降级
- ✅ **结果聚合** — 汇总所有子任务的执行结果
...
```bash
# 批量重构：将所有 Python 文件从 print 改为 logging
/batch 将所有 Python 文件中的 print 语句改为 logging.info

# 批量更新：升级所有依赖到最新版本
/batch 升级 requirements.txt 中所有依赖到最新版本，并修复兼容性问题

# 批量格式化：统一代码风格
/batch 使用 black 格式化所有 Python 文件，并修复 lint 错误
```
